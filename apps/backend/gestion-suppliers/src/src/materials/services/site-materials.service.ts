...
export class SiteMaterialsService {
  ...
  async createMaterialWithSite(
    ...
    try {
      ...
      const stockMinimum =
        createMaterialDto.stockMinimum ?? createMaterialDto.minimumStock ?? 10;
      const stockActuel =
        createMaterialDto.stockActuel ??
        stockExistant + stockEntree - stockSortie;
      ...
      this.logger.log(
        `✅ Material created with site ${siteId}: ${saved._id} (stock: ${stockActuel})`,
      );
      return saved;
    } catch (error) {
      this.logger.error(
        `❌ Error creating material with site: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getMaterialAvailabilityForSite(
    materialId: string,
    siteId: string,
  ): Promise<{
    material: Material;
    ...
    const result = await Promise.all(
      materials.map(async (material) => {
        ...
        return {
          ...
          assignedSites:
            material.assignedSites?.map((id: Types.ObjectId) => ({
              id: id.toString(),
            })) || [],
          needsReorder: material.quantity <= material.stockMinimum,
        };
  ...
  async getMaterialsByCategoryAndSite(
    category: string,
    siteId: string,
  ): Promise<Material[]> {
    if (!Types.ObjectId.isValid(siteId)) {
