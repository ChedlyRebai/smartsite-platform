...
export class ImportExportService {
  ...
  async importFromExcel(filePath: string): Promise<BulkImportResponse> {
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          this.logger.log(
            `🔄 Traitement ligne ${rowNumber}: ${row.code || 'N/A'}`,
          );
          ...
          this.logger.log(
            `✅ Ligne ${rowNumber} importée avec succès: ${material.code}`,
          );
        } catch (error) {
      ...
      this.logger.log(
        `✅ Import terminé: ${response.imported} réussis, ${response.failed} échoués`,
      );
      return response;
    } catch (error) {
  ...
  private validateRowData(row: any, rowNumber: number): BulkImportMaterialDto {
    ...
    const code =
      getValue(['code', 'codemateriau', 'reference', 'ref']) || row.code;
    const name =
      getValue(['nom', 'name', 'designation', 'libelle']) || row.name;
    const category = getValue(['categorie', 'category', 'cat']) || row.category;
    const unit = getValue(['unite', 'unit', 'unitemesure']) || row.unit;
    const quantity =
      getValue(['quantite', 'quantity', 'qte', 'stock']) || row.quantity;
    const minimumStock =
      getValue(['stockminimum', 'minimumstock', 'stockmin', 'minstock']) ||
      row.minimumStock;
    const maximumStock =
      getValue(['stockmaximum', 'maximumstock', 'stockmax', 'maxstock']) ||
      row.maximumStock;
    const stockMinimum =
      getValue([
        'pointdecommande',
    ...
    const expiryDate =
      getValue(['dateexpiration', 'expirydate', 'expiration', 'peremption']) ||
      row.expiryDate;
    const qualityGrade =
      getValue(['qualite', 'qualitygrade', 'quality']) || row.qualityGrade;
    ...
    if (
      stockMinimumNum < minimumStockNum ||
      stockMinimumNum > maximumStockNum
    ) {
  ...
  private async createMaterialFromRow(
    rowData: BulkImportMaterialDto,
  ): Promise<Material> {
    ...
    if (rowData.specifications) {
      try {
        specifications =
          typeof rowData.specifications === 'string'
  ...
  async exportToPDF(
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        doc
          .fontSize(10)
          .text(`Total: ${materials.length} matériaux`, { align: 'left' });
        doc.moveDown();
