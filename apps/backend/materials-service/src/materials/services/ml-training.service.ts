...
export class MLTrainingService {
  ...
  parseCSV(csvContent: string, materialId: string): ParsedHistoricalData {
    try {
      ...
      console.log('📄 New features - hourOfDay:', hourOfDayIndex, 'dayOfWeek:', dayOfWeekIndex, 'siteActivity:', siteActivityLevelIndex, 'weather:', weatherIndex, 'projectType:', projectTypeIndex);
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        try {
          ...
          const hourOfDay = hourOfDayIndex >= 0 && values[hourOfDayIndex] ? parseInt(values[hourOfDayIndex], 10) : undefined;
          const dayOfWeek = dayOfWeekIndex >= 0 && values[dayOfWeekIndex] ? parseInt(values[dayOfWeekIndex], 10) : undefined;
          const siteActivityLevel = siteActivityLevelIndex >= 0 && values[siteActivityLevelIndex] ? parseFloat(values[siteActivityLevelIndex]) : undefined;
          const weather = weatherIndex >= 0 ? values[weatherIndex] : undefined;
          const projectType = projectTypeIndex >= 0 ? values[projectTypeIndex] : undefined;
          ...
          data.push({ 
            ...
            siteActivityLevel: !isNaN(siteActivityLevel as number) ? siteActivityLevel : undefined,
            weather,
            projectType
          });
        } catch (e) {
  ...
  async trainModel(
    ...
    this.logger.log(`🧠 Training model for material ${materialId} with ${historicalData.length} data points, useAdvancedFeatures: ${useAdvancedFeatures}`);
    ...
    try {
      ...
      if (useAdvancedFeatures) {
        historicalData.forEach((point) => {
          const weatherEncoded = point.weather ? (this.weatherEncoding.get(point.weather.toLowerCase()) ?? 0) : 0;
          const projectTypeEncoded = point.projectType ? (this.projectTypeEncoding.get(point.projectType.toLowerCase()) ?? 0) : 0;
      ...
      const accuracy = maxStock > 0 ? Math.max(0, 1 - (finalLoss / (maxStock * maxStock))) : 0.5;
  ...
  private hasAdvancedFeatures(data: HistoricalDataPointDto[]): boolean {
    ...
    return !!(firstRow.hourOfDay !== undefined || 
  ...
  private recommendedOrderQuantity(
    ...
    const leadHours = 48;
    return Math.max(0, Math.ceil(rate * leadHours + reorderPoint - currentStock));
  }
  ...
  async predictStockAdvanced(
    ...
    if (materialModel && materialModel.useAdvancedFeatures) {
      ...
      try {
        const weatherEncoded = this.weatherEncoding.get(features.weather.toLowerCase()) ?? 0;
        const projectTypeEncoded = this.projectTypeEncoding.get(features.projectType.toLowerCase()) ?? 0;
        ...
        hoursToOutOfStock = consumptionRate > 0 ? predictedStock / consumptionRate : 999;
      } catch (error) {
        this.logger.warn('Advanced ML prediction failed, using fallback calculation');
        hoursToOutOfStock = consumptionRate > 0 ? currentStock / consumptionRate : 999;
      }
    } else {
      hoursToOutOfStock = consumptionRate > 0 ? currentStock / consumptionRate : 999;
    }

    const recommendedOrderQuantity = Math.max(0, Math.ceil(consumptionRate * 48 - currentStock));
    const estimatedRuptureDate = new Date(Date.now() + hoursToOutOfStock * 3600000).toISOString();
    const status = this.determineStatus(hoursToOutOfStock);
    const message = this.generateAdvancedMessage(status, hoursToOutOfStock, recommendedOrderQuantity);
   ...
   */
  private generateAdvancedMessage(status: 'safe' | 'warning' | 'critical', hoursToOutOfStock: number, recommendedQuantity: number): string {
    if (status === 'safe') {
