...
export class PaymentService {
  ...
  private readonly PAYMENT_API_URL =
    process.env.PAYMENT_API_URL || 'http://localhost:3008/api/payments';
  private readonly FACTURE_API_URL =
    process.env.FACTURE_API_URL || 'http://localhost:3008/api/factures';
  ...
  async createPayment(
    ...
    try {
      this.logger.log(
        `💳 Création paiement: site=${siteId}, montant=${amount}, méthode=${paymentMethod}`,
      );
  ...
  private async createStripePaymentIntent(amount: number): Promise<string> {
    try {
      const stripeApiUrl =
        ...
        'http://localhost:3008/api/payments/stripe/create-payment-intent';
      ...
      throw new BadRequestException(
        "Erreur lors de l'initialisation du paiement par carte",
      );
    }
  }
  ...
  async confirmCardPayment(
    ...
    try {
      this.logger.log(
        `✅ Confirmation paiement Stripe: paymentId=${paymentId}, stripeId=${stripePaymentIntentId}`,
      );
  ...
  async generateInvoice(paymentId: string, siteNom: string): Promise<any> {
    try {
      this.logger.log(
        `📄 Génération facture pour paiement: ${paymentId}, site: ${siteNom}`,
      );

      const response = await firstValueFrom(
        this.httpService.post(`${this.FACTURE_API_URL}/${paymentId}`, {
          siteNom,
        }),
      );
